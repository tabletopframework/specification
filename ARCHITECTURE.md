...
| `Видение` | Дерево допущений по поводу стратегических, тактических или оперативных задач. |
| `Нарратив` | История суждений по поводу хода сражения. |

| Компонент | |
| ------------- | ------------- |
| `Ментор` | Исполняется игроком. В серьёзной игре — исполняется тренером. |
| `Движок` | Хранит текущее состояние доски. Исполняется игроком. |
| `Оракул` | Хранит потенциальное состояние доски. Исполняется игроком. |
