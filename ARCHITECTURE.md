...
Каждый протокол определяет тип сообщений, которыми обмениваются игроки и компоненты. В любой конкретный момент выбор сообщения ограничен множеством доступных вариантов; игрок или компонент выбирает одно из них и отправляет через соответствующий протокол. После получения сообщение обрабатывается, и состав доступных сообщений может измениться.
...
В основе лежит различие между проактивным и реактивным поведением. Проактивность — произвольное поведение участника в свой ход. Реактивность — предзаданное поведение в ход другого участника.
...
| `Оракул` | Рассчитывает вероятность вытянуть взрывного котёнка. Отчасти представлен физически картой "Подсмуртри грядущее". Исполняется участником в свой ход. |
| `Нарратор` | Встроен в иллюстрации и юмористический текст. Усиливается комментариями самих игроков по ходу партии. Исполняется участниками без привязки к ходу. |
...
| `Арки` | Развитие фигур (развёртывание, столкновение, утрата). |

| Протокол | |
| -------- | ------------- |
| `Персонаж` | Список военных операций (замыслы в предметной области войн/сражений). |
...
| Компонент | |
| --------- | ------------- |
| `Ментор` | Вырабатывает стратегию. Исполняется участником в свой ход. В серьёзной игре — исполняется тренером. |
| `Движок` | Представляет текущее состояние доски. Исполняется участником в свой ход. |
| `Оракул` | Конструирует потенциальное состояние доски. Исполняется участником в свой ход. В серьёзной игре — исполняется секундантом. |
...
| Протокол | |
| -------- | ------------- |
| `Персонаж` | Базовые намерения [наращивания](https://boardgamegeek.com/thread/2042891/article/29771337) поддержки собственной фракции и ослабление поддержки других фракций среди [жителей](https://www.youtube.com/watch?v=dLNFWygk2Cs) леса (общая колода карт). Плюс специфичные намерения каждой фракции. |
...
| `Оракул` | Маркеры победных очков, положение которых обновляется в конце каждого хода. Треки на планшетах фракций. Исполняется участником в свой ход. |
...
- Оракул отчасти представлен физически в виде сброса общей колоды и всевозможных треков.
...
| `Механики` | Оригинальная одноименная механическая система «Пандемия». |
...
Итого:
- Все компоненты системы исполняются участниками.
