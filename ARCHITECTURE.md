...
<p align="center">
    <img src="./fig/architecture/matrix-with-protocols-plural.svg">
</p>

Среда — это набор _компонентов_, реализующих протоколы. Персонаж реализуется _ментором_, нарратив — _нарратором_, аватар — _движком_ и прогноз — _оракулом_.
...
| Компонент | |
| ------------- | ------------- |
| `Ментор` | Текущее состояние сражения. Исполняется игроком. В серьёзной игре — исполняется тренером. |
| `Движок` | Представляет текущее состояние доски. Исполняется игроком. |
| `Оракул` | Конструирует потенциальное состояние доски. Исполняется игроком. |
| `Нарратор` | Исполняется игроком. В серьёзной игре — исполняется комментатором. |

Итого:
- Два абсолютно симметричных актора.
- Все компоненты среды исполняются игроками.
- Движок является единственным компонентом, который представлен физически. Остальные компоненты представленны ментально в виде способностей, которые нарабатываются на тренировках.
