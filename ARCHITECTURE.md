...
<p align="center">
    <img src="./fig/architecture/player-protocol-environ.svg">
</p>
...
В любой игре игроку противостоит _среда_. Среда — это набор _компонентов_, реализующих протоколы. Компоненты могут исполняться любыми участниками игры. Например, в соло игре без приложения компоненты среды исполняются игроком, в НРИ - игромастером.
...
Итого:
- Два абсолютно симметричных актора.
...
| Компонент | |
| --------- | ------------- |
| `Ментор` | Вырабатывает стратегию борьбы с эпидемией. Исполняется игроками коллективно. |
| `Движок` | Представляет текущее состояние поля. Исполняется игроками поочередно. |
| `Оракул` | Рассчитывает вероятности выхода карт. Исполняется игроками коллективно. |
| `Нарратор` | Фиксирует происходящее в терминах темы и сеттинга. Исполняется игроками поочередно. |

Итого:
- Игроки играют против среды, все компоненты которой исполняются игроками.
