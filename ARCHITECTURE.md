...
Это различие определяет две позиции, которые может занять участник: _игрок_ и _среда_. Игрок всегда проактивен, а среда — реактивна. Один и тот же участник может занимать обе позиции. Разные участники могут занимать одну и ту же позицию.
...
| Компонент | |
| --------- | ------------- |
| `Ментор` | Вырабатывает стратегию. Исполняется каждым участником в свой ход. В серьёзной игре — исполняется тренером. |
| `Движок` | Представляет текущее состояние доски. Исполняется каждым участником в свой ход. |
| `Оракул` | Конструирует потенциальное состояние доски. Исполняется каждым участником в свой ход. В серьёзной игре — исполняется секундантом. |
| `Нарратор` | Объясняет происходящее. Исполняется каждым участником в свой ход. В серьёзной игре — исполняется комментатором. |
...
| `Движок` | Представляет текущее состояние поля. Исполняется каждым участником в свой ход. |
| `Оракул` | Рассчитывает вероятности выхода карт. Исполняется всеми участниками. |
| `Нарратор` | Фиксирует происходящее в терминах темы и сеттинга. Исполняется каждым участником в свой ход. |
